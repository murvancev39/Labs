...
int main ()
{
    ...
    FILE *first = fopen ("first_test.txt", "w");
    assert (first);
    FILE *second = fopen ("second_test.txt", "w");
    assert (second);
    FILE *third = fopen ("third_test.txt", "w");
    assert (third);
    FILE *fourth = fopen ("fourth_test.txt", "w");
    assert (fourth);
...
double test1 (int num_elements)
{
    ...
    {
        struct Stack *stack_ptr = stack_ctr (1, sizeof (int));
